...
function the_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			previous_post_link( '<div class="nav-previous single-post-prev">' . $prevthumbnail . '%link</div>', _x( '%title', 'Previous post link', 'gisikurath' ) );
            next_post_link('<div class="nav-next single-post-next">' . $nextthumbnail . '%link</div>',  _x( '%title', 'Next post link',     'gisikurath' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function gisikurath_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'gisikurath' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'gisikurath' ),
...
function gisikurath_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && gisikurath_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( '%1$s', 'gisikurath' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'gisikurath' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'gisikurath' ) . '</span>', $tags_list );
		}
	}
